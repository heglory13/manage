...
exports.buildInventoryValuationBuckets = buildInventoryValuationBuckets;
const index_1 = require("@prisma/client/index");
function roundCurrency(value) {
    return Math.round((value + Number.EPSILON) * 100) / 100;
}
function buildInventoryValuationBuckets(rows, startDate, endDate) {
    const activeRows = rows
        .filter((row) => row.status === index_1.InventoryTransactionStatus.ACTIVE)
        .sort((a, b) => {
    ...
    for (const row of activeRows) {
        ...
        const isInPeriod = (!startDate || row.createdAt >= startDate) &&
            (!endDate || row.createdAt <= endDate);
        if (row.type === index_1.TransactionType.STOCK_IN) {
    ...
    return Array.from(bucketMap.values())
        ...
        .filter((bucket) => bucket.openingQty !== 0 ||
        bucket.totalInQty !== 0 ||
        bucket.totalOutQty !== 0 ||
        bucket.closingQty !== 0);
}
//# sourceMappingURL=inventory-valuation.util.js.map