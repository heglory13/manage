...
export class PreliminaryCheckService {
  ...
  async findAll(
    ...
    if (filters.status && filters.status.trim() !== '') {
      const validStatuses = Object.values(PreliminaryCheckStatus) as string[];
    ...
    if (filters.quantity?.trim()) {
      where.quantity = Number(filters.quantity.trim());
    }
    if (filters.creator?.trim()) {
      const names = filters.creator.split(',').map((s) => s.trim()).filter(Boolean);
      if (names.length === 1) {
        where.creator = { name: { contains: names[0] } };
      } else {
        where.creator = { OR: names.map((n) => ({ name: { contains: n } })) };
      }
    }
