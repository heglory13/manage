...
export class WarehouseController {
  ...
  @Patch('positions/:id/toggle-storage-location')
  async toggleStorageLocation(
    @CurrentUser() currentUser: Record<string, unknown>,
    @Param('id') id: string,
  ) {
    const user = currentUser as unknown as UserPayload;
    this.checkPermission(user, 'edit');
    return this.warehouseService.toggleStorageLocation(id);
  }
  ...
  @Post('positions/resync-all')
  @Roles(Role.ADMIN)
  async resyncAllPositions(@CurrentUser() currentUser: Record<string, unknown>) {
    const user = currentUser as unknown as UserPayload;
    this.checkPermission(user, 'edit');
    return this.warehouseService.resyncAllPositions();
  }

  @Post('positions/sync-capacities')
  @Roles(Role.ADMIN)
  async syncPositionCapacities(@CurrentUser() currentUser: Record<string, unknown>) {
    const user = currentUser as unknown as UserPayload;
    this.checkPermission(user, 'edit');
    return this.warehouseService.syncAllMaxCapacities();
  }
}
