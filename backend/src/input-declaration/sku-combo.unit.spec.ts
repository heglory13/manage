      ...
      (prisma.skuCombo as Record<string, jest.Mock>).findMany.mockResolvedValue([
        {
          ...
          inventoryTransactions: [
            { category: { id: 'cat-1', name: 'Áo thun' } },
          ],
        },
      ]);
      (prisma.skuCombo as Record<string, jest.Mock>).count.mockResolvedValue(1);

      const result = await service.getAll({ search: 'den' });

      expect(result.total).toBe(1);
      expect(result.data).toHaveLength(1);
      expect((result.data[0] as Record<string, unknown>).color).toEqual({
        id: 'c2',
        name: 'Đen',
      });
    });
