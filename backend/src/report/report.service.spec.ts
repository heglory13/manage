...
function createMockInventoryService() {
  return {
    stockInBatch: jest.fn().mockResolvedValue({ success: true, importedRows: 0, totalQuantity: 0, transactions: [] }),
  };
}

function createMockSkuComboService() {
  return {
    findOrCreate: jest.fn().mockResolvedValue({ id: 'mock-combo-id' }),
  };
}
...
describe('ReportService', () => {
  it('getNxtReport should aggregate by categoryId and categoryName', async () => {
    ...
    const service = new ReportService(
      ...
      createMockInventoryService() as unknown as InventoryService,
      createMockSkuComboService() as unknown as SkuComboService,
    );
