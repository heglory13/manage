...
export function useSavedFilters({ pageKey, onFiltersChange, initialFilters }: UseSavedFiltersOptions) {
  const [filters, setFilters] = useState<Record<string, unknown>>(initialFilters ?? {});
  const [draftFilters, setDraftFilters] = useState<Record<string, unknown>>(initialFilters ?? {});
  ...
  return {
    ...
    activeFilterId,
    isLoading,
    // Filters are now applied immediately on selection, so there are never
    // pending changes waiting for an explicit "Apply" click.
