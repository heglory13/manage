...
export default function OrderPlansPage() {
  ...
  const fetchRows = useCallback(async (page = opPage) => {
    ...
    try {
      ...
      const res = await api.get('/order-plans', {
        params: {
          limit: opPageSize,
          page,
          type: sf(f.type),
          ...
          customerName: sf(f.customerName),
  ...
  }, [opPageSize, opPage, savedFilterHook.filters]);
  ...
  const exportToExcel = useCallback(async (includeFilters: boolean) => {
    ...
    try {
      ...
      if (includeFilters) {
        const f = savedFilterHook.filters;
        params.type = sf(f.type);
        params.status = sf(f.status);
        params.category = sf(f.category);
        params.customerName = sf(f.customerName);
        params.dateFrom = sf(f.dateFrom);
        params.dateTo = sf(f.dateTo);
      }
      const res = await api.get('/order-plans', { params });
      const data = (res.data.data || []) as OrderPlanRow[];
      if (data.length === 0) {
      ...
      XLSX.writeFile(wb, `ke-hoach-dat-hang-${mode}-${new Date().toISOString().slice(0, 10)}.xlsx`);
      setExportOpen(false);
    } catch (error: any) {
  ...
  return (
    <AppLayout>
              ...
              <Table className="border-none">
                <TableHeader>
                  <TableRow className="hover:bg-transparent">
                    ...
                    <TableHead>Nguoi tao phieu</TableHead>
                ...
                {filteredRows.map((item) => (
                  <div key={item.id} className="px-4 py-4">
                    ...
                    <div className="space-y-0.5 text-xs text-slate-500 mb-3">
                      ...
                      <div className="flex gap-2"><span className="w-20 shrink-0">Người tạo</span><span>{item.creator?.name || '-'}</span></div>
      ...
      <TransactionExportDialog
        ...
        title="Xuất Excel kế hoạch đặt hàng"
