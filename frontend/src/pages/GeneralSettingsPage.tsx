...
export default function GeneralSettingsPage() {
  ...
  const handleDownloadBackup = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `backup-database-${new Date().toISOString().slice(0, 10)}.json`;
      ...
      alert('Khong the tai backup du lieu luc nay.');
    } finally {
              ...
              <div className="grid gap-10 lg:grid-cols-[340px_minmax(0,1fr)]">
                ...
                <div className="rounded-[24px] bg-slate-50 p-6">
                  <div className="rounded-[20px] border border-slate-200 bg-white p-5 shadow-sm">
                    <div className="text-[22px] font-semibold text-slate-900">Tai backup database</div>
                    <div className="mt-2 text-[16px] text-slate-500">
                      Nen backup dinh ky truoc khi cap nhat he thong hoac thay doi du lieu lon.
                    </div>
                    <div className="mt-5">
                      <Button variant="outline" onClick={handleDownloadBackup} disabled={isDownloadingBackup}>
                        <Download size={16} />
                        {isDownloadingBackup ? 'Dang tao file backup...' : 'Tai backup ve may'}
                      </Button>
                    </div>
                  </div>
                </div>
