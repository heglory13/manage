...
export default function PreliminaryChecksPage() {
  ...
  const savedFilterHook = useSavedFilters({
    pageKey: 'preliminary-checks',
    onFiltersChange: () => setPcPage(1),
  });
    ...
    return [
      {
        ...
        type: 'select',
        options: [
          { value: 'PENDING', label: 'Chờ kiểm tra chi tiết' },
          { value: 'APPROVED,REJECTED', label: 'Đã kiểm tra chi tiết' },
        ],
      },
    {
      key: 'category',
  ...
  const fetchRows = useCallback(async (page = pcPage) => {
    ...
    try {
      ...
      const res = await api.get('/preliminary-checks', {
        params: {
          limit: pcPageSize,
          page,
          status: sf(f.status),
          category: sf(f.category),
          quantity: sf(f.quantity),
          creator: sf(f.creator),
  ...
  }, [pcPageSize, pcPage, savedFilterHook.filters]);
  ...
  const exportToExcel = useCallback(async (includeFilters: boolean) => {
    setIsExporting(true);
    try {
      const params: Record<string, any> = { limit: 100000, page: 1 };
      if (includeFilters) {
        const f = savedFilterHook.filters;
        params.status = sf(f.status);
        params.category = sf(f.category);
        params.quantity = sf(f.quantity);
        params.creator = sf(f.creator);
        params.dateFrom = sf(f.dateFrom);
        params.dateTo = sf(f.dateTo);
      }
      const res = await api.get('/preliminary-checks', { params });
      const data = (res.data.data || []) as PreliminaryCheckRow[];
      if (data.length === 0) {
      ...
      const excelData = data.map((row) => ({
        ...
        'Danh mục SP': getRowCategoryLabel(row),
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Kiểm sơ bộ');
      const mode = includeFilters ? 'filtered' : 'all';
      XLSX.writeFile(wb, `kiem-so-bo-${mode}-${new Date().toISOString().slice(0, 10)}.xlsx`);
      setExportOpen(false);
    } catch (error: any) {
  ...
  return (
    <AppLayout>
      ...
      <TransactionExportDialog
        ...
        title="Xuất Excel kiểm sơ bộ"
        onExportAll={() => exportToExcel(false)}
        onExportFiltered={() => exportToExcel(true)}
