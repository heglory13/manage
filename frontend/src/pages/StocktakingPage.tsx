...
import { formatDateTime, formatNumber, matchSel, sf } from '../lib/utils';
...
export default function StocktakingPage() {
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const statusVal = sf(f.status);
      if (statusVal) params.status = statusVal;
      const modeVal = sf(f.mode);
      if (modeVal) params.mode = modeVal;
      const dateVal = sf(f.date);
      if (dateVal) {
        params.startDate = dateVal;
        params.endDate = dateVal;
      }
