...
import { Check, CheckCircle2, Clock3, Eye, History, Plus, Printer, Search, ShieldAlert, Upload, X } from 'lucide-react';
...
export default function StocktakingPage() {
  ...
  const [signedDocument, setSignedDocument] = useState<{ name: string; preview: string } | null>(null);
  ...
  const handleCreate = async () => {
    try {
      const payload: { mode: 'full' | 'selected'; productIds?: string[] } = { mode: createMode };
    ...
    try {
      ...
      const items = selectedRecord.items.map((item) => {
        ...
        return {
          ...
          evidenceUrl: signedDocument?.preview || undefined,
        };
      });
      ...
      alert(err.response?.data?.message || 'Khong the xac nhan so lieu kiem ke');
    } finally {
        ...
        <div class="wrap">
          <div class="head">
            <div>
              ...
              <div class="sub">${COMPANY_INFO.phone}</div>
            </div>
            <div class="right">
                  ...
                  <div className="space-y-3">
                    {selectedRecord.items.map((item) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className="grid gap-4 rounded-[24px] border border-slate-200 bg-white p-4 shadow-sm lg:grid-cols-[minmax(260px,1.7fr)_140px_240px_minmax(220px,1.4fr)] lg:items-center"
                        >
                          <div>
            ...
            <div className="px-6 py-6">
              <div className="mx-auto w-full max-w-[900px] rounded-[10px] border border-slate-200 bg-white px-10 py-8 shadow-sm">
                <div className="grid gap-6 md:grid-cols-[minmax(0,1fr)_280px]">
                  <div>
                    <div className="text-[22px] font-bold uppercase text-slate-950">{COMPANY_INFO.name}</div>
                    <div className="mt-1 text-[15px] text-slate-700">{COMPANY_INFO.address}</div>
                    <div className="text-[15px] text-slate-700">{COMPANY_INFO.phone}</div>
                  </div>
                      ...
                      {selectedRecord.items.map((item, index) => {
                        ...
                        return (
                          <tr key={item.id}>
                            ...
                            <td className="border border-slate-500 px-3 py-3">{noteDisplay}</td>
                          </tr>
                        );
                      })}
                    </tbody>
