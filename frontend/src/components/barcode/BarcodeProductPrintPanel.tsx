...
interface BarcodeProductPrintPanelProps {
  embedded?: boolean;
  onCreated?: () => void;
}

export function BarcodeProductPrintPanel({ embedded = false, onCreated }: BarcodeProductPrintPanelProps) {
  ...
  const [paperSize, setPaperSize] = useState<PaperSize>('72x22');
  ...
  const addItem = (combo: any) => {
    const productName = [combo.classification?.name, combo.color?.name, combo.size?.name, combo.material?.name]
      .filter(Boolean)
      .join(' - ');
    const exists = items.find((item) => item.sku === combo.compositeSku);

    if (exists) {
      setItems((prev) =>
        prev.map((item) => (item.sku === combo.compositeSku ? { ...item, quantity: item.quantity + 1 } : item)),
      );
    } else {
      setItems((prev) => [
        ...prev,
  ...
  const handleCreateTicket = async () => {
    const validItems = items.filter((item) => item.quantity > 0 && item.salePrice > 0);
    if (validItems.length === 0) {
      alert('Vui lòng thêm ít nhất 1 sản phẩm với số lượng và giá bán > 0.');
      return;
    }

    // Save to print history (auto-approved)
    try {
  ...
  const previewItem = items[0] || {
    productName: 'Áo thun 30/4 Rồng tiên Cotton 100% - Size 2 - Trắng',
    sku: 'TS57CMT-2',
    salePrice: 398000,
  };
          ...
          {searchOpen && searchResults.length > 0 && (
            <div className="absolute left-0 right-0 top-full z-50 mt-1 max-h-60 overflow-auto rounded-xl border border-slate-200 bg-white shadow-lg">
              {searchResults.map((combo: any) => {
                const label = [combo.classification?.name, combo.color?.name, combo.size?.name, combo.material?.name]
                  .filter(Boolean)
                  .join(' - ');
                return (
                  <button
                  ...
                  >
                    ...
                    <div className="font-mono text-xs text-slate-500">{combo.compositeSku}</div>
                  </button>
                );
              })}
            </div>
          ...
          <CardContent>
            <div className="grid gap-6 md:grid-cols-[200px_1fr]">
              ...
              <div className="flex items-center justify-center rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 p-6">
                <div
                  className="flex flex-col items-center justify-center overflow-hidden border border-slate-400 bg-white text-center"
                  style={{
                ...
                >
                  {showName && (
                    <div className="max-h-[24px] w-full overflow-hidden text-[9px] font-medium leading-tight">
                  ...
                  {showBarcode && (
                    <>
                      ...
                      <div className="mt-0.5 text-[8px] font-semibold">{previewItem.sku}</div>
                    </>
                  )}
        ...
        <Card className="rounded-2xl">
          ...
          <CardContent className="space-y-4">
            {[
              ...
              const sizes = PAPER_SIZES.filter((paper) => paper.cols === group.cols);
                    ...
                    {sizes.map((paper) => (
                      <label
                        ...
                        className={`flex cursor-pointer items-center gap-3 rounded-xl border px-4 py-3 transition ${
                          paperSize === paper.value ? 'border-indigo-400 bg-indigo-50' : 'border-slate-200 hover:bg-slate-50'
                        }`}
                      >
                        <input
                          type="radio"
                          name="paperSize"
                          checked={paperSize === paper.value}
                          onChange={() => setPaperSize(paper.value)}
                        />
