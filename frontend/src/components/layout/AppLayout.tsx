...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="app-shell">
      {mobileNavOpen && <button className="ims-mobile-overlay" onClick={() => setMobileNavOpen(false)} aria-label="Dong dieu huong" />}
      ...
      <aside className={`ims-sidebar ${mobileNavOpen ? 'mobile-open' : ''}`}>
        <div className="ims-brand">
          ...
          <button className="ims-mobile-close" onClick={() => setMobileNavOpen(false)} aria-label="Dong menu">
            <X size={18} />
          </button>
        </div>
